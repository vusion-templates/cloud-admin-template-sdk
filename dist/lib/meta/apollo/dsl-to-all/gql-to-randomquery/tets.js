...
exports.generateRandomMutation = exports.generateRandomQuery = exports.getTypeName = void 0;
...
function getSelectionSetAndVars(schema, node, config, depth = 0) {
    ...
    if (node.kind === graphql_1.Kind.OBJECT_TYPE_DEFINITION) {
        ...
        fields.forEach((field) => {
            ...
            // 根据查询名称设置
            selectionsMap[field.name.value] = [selection];
        });
    }
        ...
        fields.forEach((field) => {
            ...
            selections.push(selection);
            // 对象映射
            if ((selectionsMap[field.name.value] || []).length > 0) {
                selectionsMap[field.name.value].push(selection);
            }
            else {
                selectionsMap[field.name.value] = [selection];
            }
    ...
    const { aliasIndexes, cleanselections } = UninNames(selections);
    return {
        selectionSet: //  全部的 query field
...
function UninMap(selectionsMap) {
    const result = {};
    Object.keys(selectionsMap).forEach((fieldKey) => {
        const { aliasIndexes, cleanselections } = UninNames(selectionsMap[fieldKey]);
        result[fieldKey] = {
...
function getQueryOperationDefinition(schema, config) {
    ...
    console.info('selectionSet', JSON.stringify(selectionSet));
    console.info('variableDefinitionsMap', JSON.stringify(variableDefinitionsMap));
    return {
        queryDocument: {
