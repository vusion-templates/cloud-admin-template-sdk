import { DSLSchema, isOa3Param, Oa3Param, Responses, GraphQLParameters, OA3BodyParam, getParamDetailsFromRequestBody, Endpoints, Oa2NonBodyParam } from "./confg";
...
export const getParamDetails = (param: Oa2NonBodyParam): StructureParam  => {
...
export const getServerPath = (schema: DSLSchema): string | undefined => {
  const server =
...
export const getAllInterfaces = (schema: DSLSchema) => {
  ...
  const serverPath = getServerPath(schema); // get server path
  Object.keys(schema.interfaces).forEach((interfacePath: string) => {
    ...
    Object.keys(route).forEach((method: string) => {
      ...
      const parameterDetails = [
        ...(operationObject.parameters
          ? operationObject.parameters.map((param: Oa3Param) => getParamDetails(param))
          : []),
        ...bodyParams,
      ];
