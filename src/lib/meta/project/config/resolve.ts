...
function getConfig(cwd: string, configPath: string, packagePath: string): any {
  ...
  if (fs.existsSync(configPath)) return require(configPath);
    ...
    if (packageVusion) return packageVusion;
    else {
      throw new Error(
        chalk.bgRed(" ERROR ") +
          ` Cannot find vusion config! This is not a vusion project.
    processCwd: ${cwd}
...
export default function resolve(
   ...
   */
  if (args) {
    if (args["vusion-mode"]) config.mode = args["vusion-mode"];
    ...
    if (args["src-path"]) config.srcPath = path.resolve(cwd, args["src-path"]);
  ...
  if (config.type === "library") {
    ...
    if (!libraryName && pkg.peerDependencies)
      libraryName = Object.keys(pkg.peerDependencies).find((key) =>
        key.endsWith(".vusion")
      );
    if (!libraryName) libraryName = "cloud-ui.vusion";
  ...
  if (Array.isArray(config.theme)) {
    ...
    config.theme.forEach((_theme) => {
      if (_theme.endsWith(".css")) {
        // is a path
        let name = path.basename(_theme, ".css");
        if (name === "theme") name = "default";
        ...
        else theme[_theme] = path.resolve(cwd, `./themes/${_theme}.css`);
      }
    });
  ...
  if (themeAutoDetected) {
    ...
    if (!fs.existsSync((config.theme as Theme).default))
      (config.theme as Theme).default = path.resolve(
        config.libraryPath,
        "./base/global.css"
      );
    ...
    if (!fs.existsSync((config.theme as Theme).default)) {
      try {
        (config.theme as Theme).default = path.resolve(
          require.resolve("@vusion/doc-loader"),
          "../node_modules/proto-ui.vusion/src/styles/theme.css"
        );
      } catch (e) {
        // not work
      }
  ...
  if (!config.baseCSSPath) {
    baseCSSPath = config.baseCSSPath = path.resolve(
      config.libraryPath,
      "./styles/base.css"
    );
    ...
    if (!fs.existsSync(config.baseCSSPath)) {
      try {
        config.baseCSSPath = path.resolve(
          require.resolve("@vusion/doc-loader"),
          "../node_modules/proto-ui.vusion/src/styles/base.css"
        );
      } catch (e) {
        // not work
      }
